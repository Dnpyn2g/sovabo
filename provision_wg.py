...
LOG_PATH = os.path.join(ART_DIR, 'provision.log')
...
if command -v apt-get >/dev/null 2>&1; then
  ...
  apt-get update -y
  apt-get install -y wireguard qrencode iproute2 iptables curl zip
elif command -v dnf >/dev/null 2>&1; then
  dnf install -y wireguard-tools qrencode iproute iptables curl zip || true
elif command -v yum >/dev/null 2>&1; then
  yum install -y epel-release || true
  yum install -y wireguard-tools qrencode iproute iptables curl zip || true
elif command -v pacman >/dev/null 2>&1; then
  pacman -Sy --noconfirm wireguard-tools qrencode iproute2 iptables curl zip || true
fi
...
# Prepare dirs
rm -rf /root/wg-clients || true
mkdir -p /root/wg-clients

# Write server interface config header (without clients)
...
async def main():
    ...
    async with aiosqlite.connect(args.db, timeout=30) as db:
        cur = await db.execute(
            "SELECT server_host, server_user, server_pass, ssh_port, config_count FROM orders WHERE id=?",
            (args.order_id,)
        )
    ...
    if not row:
        ...
        sys.exit(3)
    host, user, passwd, port, cfg_count = row
    ...
    try:
        ...
        with client.open_sftp() as sftp:
            remote_script = f"/tmp/setup_wg_{args.order_id}.sh"
            with sftp.file(remote_script, 'w') as f:
                f.write(WG_SETUP_SCRIPT)
            sftp.chmod(remote_script, 0o700)

            ip_base = f"10.8.{(args.order_id % 200) or 1}"
            wrapper_path = f"/tmp/run_wg_{args.order_id}.sh"
            wrapper = (
                "#!/bin/bash\n"
                "set -e\n"
        ...
        # Build command (sudo if not root)
        is_root = (user or 'root').lower() == 'root'
        if is_root:
            cmd = f"bash -lc 'bash {wrapper_path}'"
        else:
            # Use -p '' to suppress sudo password prompt text
            cmd = f"bash -lc 'sudo -S -p '' bash {wrapper_path}'"

        logger.info("Run remote setup script: %s", cmd)
        ...
        if not is_root:
            try:
                ...
                logger.warning(f"provision_wg stdin.write error: {e}")

        out = stdout.read().decode('utf-8', errors='ignore')
        err = stderr.read().decode('utf-8', errors='ignore')
        code = stdout.channel.recv_exit_status()
        logger.info("Remote script exit code: %s", code)
        ...
        # Parse artifact line from stdout
        artifact = None
        for line in out.splitlines():
            if line.startswith('ARTIFACT='):
                ...
                break
        if not artifact:
            artifact = f'/etc/wireguard/wg0.conf'

        # Not downloading server config (root-only); just mark order as provisioned and store ip_base
        async with aiosqlite.connect(args.db, timeout=30) as db:
        ...
        try:
            ...
            logger.error(f"provision_wg update_order_artifact in error handler failed: {ex}")
        sys.exit(5)
    finally:
        try:
            client.close()
        except Exception as e:
            logger.warning(f"provision_wg client.close error: {e}")
