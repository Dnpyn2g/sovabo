...
logger = logging.getLogger('manage-awg')
...
async def main():
    ...
    user = user or 'root'
    port = port or 22
    ip_base = ip_base or f"10.9.{(args.order_id % 200) or 1}"
    ...
    try:
        ...
        with client.open_sftp() as sftp:
            if args.cmd == 'add':
                add_path = f"/tmp/awg_add_{args.order_id}.sh"
        ...
        if not is_root:
            try:
                ...
                logger.warning(f"manage_awg stdin.write error: {e}")
        ...
        try:
            ...
            logger.warning(f"manage_awg client.close error: {e}")
